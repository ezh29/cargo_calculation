...
function container_css(radioVal) {
  ...
  $(".container_info1").html(
    radioVal + " " + container[0] + " * " + container[1] + " * " + container[2]
  );
}
...
function chk_boxs_hw() {
  ...
  for (i = 0; i < length - 1; i++) {
    ...
    for (j = 0; j < length - 1 - i; j++) {
      ...
      if (boxs_height[j] < boxs_height[j + 1]) {
        // 두 수를 비교하여 앞 수가 뒷 수보다 작으면
  ...
  for (i = 0; i < length - 1; i++) {
    ...
    for (j = 0; j < length - 1 - i; j++) {
      // 끝까지 돌았을 때 다시 처음부터 비교하기 위한 반복문
      if (boxs_width[j] < boxs_width[j + 1]) {
        // 두 수를 비교하여 앞 수가 뒷 수보다 작으면
  ...
  if (boxs_height[0] > container_set[2]) {
    ...
    alert(
      "설정되어있는 단수*박스높이가 변경하려는 컨테이너의 고보다 높습니다."
    );
...
function drag_start() {
  ...
  $(".box_inner").droppable({
    drop: function (event, ui) {
      ...
      $(this).addClass("ui-state-highlight").addClass("bg_red");
    },
    out: function (event, ui) {
      $(this).addClass("ui-state-highlight").removeClass("bg_red");
    },
    tolerance: "touch",
  });
}
...
function tooltip_start() {
  ...
  //컨테이너 밖으로 나가면 툴팁 닫음
  $("#container_area").on("mouseleave focusout", function () {
...
function tooltip_triger_ho_btn(idx) {
  idx = idx + 1;
  $(
    "#boxlist li:nth-of-type(" + idx + ") .glyphicon-resize-horizontal"
  ).trigger("click");
}

function tooltip_triger_ver_btn(idx) {
  idx = idx + 1;
  $("#boxlist li:nth-of-type(" + idx + ") .glyphicon-resize-vertical").trigger(
    "click"
  );
}
...
function addBoxValue_btn() {
  ...
  if (
    ...
    new_box[4] >= new_box[9] &&
    new_box[9] * new_box[3] > container[2]
  ) {
    //수량이 최대단수보다 같거나 크고(최대단수 쓸때), 박스고*최대단수가 컨테이너 높이보다 높을때
    //최대단수*박스높이가 컨테이너보다 높은지 체크
    alert("최대단수*박스 높이가 컨테이너 높이를 초과합니다.");
  } else if (
    new_box[4] < new_box[9] &&
    new_box[4] * new_box[3] > container[2]
  ) {
    //수량이 최대단수보다 작고(최대단수 쓸일 없을때), 박스고*최대단수가 컨테이너 높이보다 높을때
    //박스고*수량이 컨테이너 높이보다 높은지 체크
    alert("수량*박스 높이가 컨테이너 높이를 초과합니다.");
  } else {
    //단수와 나머지 박스 계산
    //  [0]박스이름,  [1]장,  [2]폭,   [3]고, [4]수량,[5]단, [6]단 묶음수 , [7]묶음 나머지,[8] 비었음, [9]다단적재 [10]CBM
    var dan = parseInt(container[2] / new_box[3]); //컨테이너 높이 나누기 물건 높이 = 최대 단수
    if (dan >= 1) {
      //1단 이상으로 쌓을수 있을때 = 적재가능
      if (new_box[9] != "0") {
        //다단적재가 0이 아니고, 높이 *최대단수가 컨테이너 높이보다 작으면 단을 최대단수로 입력
        dan = new_box[9];
...
function boxincontainer(new_box, idx, dan) {
  ...
  var garosero_btn =
    "<span class='glyphicon glyphicon-resize-horizontal change_size' onclick='tooltip_triger_ho_btn(" +
    idx +
    ");' box_idx='" +
    idx +
    "'></span><span class='glyphicon glyphicon-resize-vertical change_size' onclick='tooltip_triger_ver_btn(" +
    idx +
    ");' box_idx='" +
    idx +
    "'></span><br/>";
  ...
  var box_html =
    ...
    '" box_dan="' +
    dan +
    '"><div class="box_inner" style="width:' +
    (new_box[2] - 2) +
    "px; height:" +
    (new_box[1] - 2) +
    'px;">' +
...
function box_list_init() {
  ...
  for (var i = 0; i < box.length; i++) {
    var dansu =
      '<span class="badge" style="width:40px;"> ' + box[i][9] + "단</span>";
    var box_name = "";
    //조건따라 단수유무, 배경색 설정
    if (box[i][9] != "0" && box[i][9] > box[i][4]) {
      //최대단수 설정이 수량보다 크면 뱃지 비활성화
      dansu =
        '<span class="badge" style="background-color:#bbb; width:40px;"> ' +
        box[i][9] +
        "단</span>";
    } else if (box[i][9] != "0") {
    } else {
      //최대단수 설정이 없으면
      dansu =
        '<span class="badge" style="background-color:#fff;color:#414344;border: 1px solid #414344; width:40px;">자동</span>';
    }
    ...
    //장*폭*고*수량
    var CBM =
      box[i][1] * 0.01 * (box[i][2] * 0.01) * (box[i][3] * 0.01) * box[i][4];
    CBM = Math.floor(CBM * 100) / 100;
    //        all_CBM = all_CBM + CBM;
    //        all_CBM = Math.floor(all_CBM * 100) / 100;

    var append =
      '<li class="ui-state-default" style="border-color:rgb(' +
      box[i][1] +
      "," +
      box[i][2] +
      "," +
      ...
      dansu +
      //위아래 버튼
      '<span class="glyphicon glyphicon-triangle-top change_size" aria-hidden="true" onclick="ch_up_btn(' +
      i +
      ');"></span>' +
      '<span class="glyphicon glyphicon-triangle-bottom change_size" aria-hidden="true" onclick="ch_down_btn(' +
      i +
      ');"></span>' +
      //장폭고 버튼
      '<span class="glyphicon glyphicon-resize-horizontal change_size" aria-hidden="true"onclick="ch_garo_btn(' +
      i +
      ');"></span>' +
      '<span class="glyphicon glyphicon-resize-vertical change_size" aria-hidden="true" onclick="ch_sero_btn(' +
      i +
      ');"></span>' +
      //삭제버튼
      '<span class="glyphicon glyphicon-remove" aria-hidden="true" onclick="box_list_delet_btn(' +
      i +
      ')"></span> ' +
      "</div>" +
      "</li>";
...
function get_boxs_heigth() {
  ...
  for (i = 0; i < length - 1; i++) {
    ...
    for (j = 0; j < length - 1 - i; j++) {
      // 끝까지 돌았을 때 다시 처음부터 비교하기 위한 반복문
      if (box_position_width[j].right < box_position_width[j + 1].right) {
        // 두 수를 비교하여 앞 수가 뒷 수보다 작으면
  ...
  for (i = 0; i < length - 1; i++) {
    ...
    for (j = 0; j < length - 1 - i; j++) {
      // 끝까지 돌았을 때 다시 처음부터 비교하기 위한 반복문
      if (box_position_height[j].bottom < box_position_height[j + 1].bottom) {
        // 두 수를 비교하여 앞 수가 뒷 수보다 작으면
  ...
  for (i = 0; i < box_position_height.length; i++) {
    box_height[i] =
      box[box_position_height[i].box_idx][3] * box_position_height[i].box_dan;
  ...
  if (box_position_width.length > 0 && box_position_height.length > 0) {
    //너비 높이는 맨 오른쪽 - 맨 왼쪽
    full_width =
      box_position_width[0].right -
      box_position_width[box_position_width.length - 1].left;
    //소수점 반올림
    full_width = Math.floor(full_width * 100) / 100;

    full_height =
      box_position_height[0].bottom -
      box_position_height[box_position_height.length - 1].top;
  ...
  $(".full_box").html(
    " 박스 너비 " + full_height + " * " + full_width + " * " + box_height[0]
  );
  ...
  for (var i = 0; i < box_position_height.length; i++) {
    var box_cbm =
      box_position_height[i].height *
      0.01 *
      (box_position_height[i].width * 0.01) *
      (box_position_height[i].go * 0.01) *
      box_position_height[i].box_dan; //(장*0.01)*(폭*0.01)*(고*0.01)*단
